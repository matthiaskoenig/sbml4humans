...
**Keys.** Every entry has one key, used by the links of the details, by the route and by the frontend: `types/Species`, `types/Species/initialAmount`, `links/compartment`, `concepts/primaryKey`, `datatypes/SIdRef`. The segment of an attribute is its key in the glossary (the field of the report, `initialAmount`, `fbc.charge`), not its label.
...
**`report/glossary.ts`** gains the keys: `typeKey(type)`, `attributeKey(type, field)`, `linkKey(kind)`, `conceptKey(key)`, `datatypeKey(name)`. `attributeKey` resolves with the chain `attributeEntry` uses (the field on the type, on `SBase`, then the first segment of a dotted field on both), and both share one lookup, so that a label and its dialog can never disagree. A key is `undefined` where there is no entry, and such a label is not clickable.

**`report/query.ts`**: `ViewState.help: string | null`, the query parameter `help`. Opening an entry from the page pushes a route, navigating inside the dialog pushes as well, closing removes the parameter. A key which is no entry closes the dialog, like a pk which is no element.

**`report/glossaryDetails.ts`**: `loadGlossaryDetails()`, one cached promise of the dynamic import of `glossary-details.json`; a rejected promise is dropped so that the next open tries again. `report/helpMarkdown.ts`: `renderHelpMarkdown(markdown)`, `markdown-it` (html off, linkify off) through DOMPurify, a `glossary:` link rendered as `<a data-help-key="...">` with the href of the route, every other link with `target="_blank" rel="noopener"`. `markdown-it` (MIT) is imported by this module only, which the dialog imports dynamically.
...
- `HelpButton.vue`: the small help icon (Lucide `CircleHelp`) for the places where a click is taken.
