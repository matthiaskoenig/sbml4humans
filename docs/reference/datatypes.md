...
Decimal digits with an optional leading sign and nothing after them. The core specification calls this type `int` and holds it to the range of a signed 32 bit number, from -2147483648 to 2147483647; the packages write `integer` for a value of the same kind, the initial level of a [qualitative species](qualitativespecies.md) or the result level of a [function term](functionterm.md). A whole number is exact and, unlike a [double](#double), carries neither a fractional part nor an exponent, which is what makes it the type of a count and of a level.
...
An attribute of type `SIdRef` holds the [SId](#sid) of an element of the model it is written in: the compartment of a species names a compartment, the variable of a rule names the element the rule computes, the conversion factor of a species names a parameter. Which kind of element a given attribute may name is no part of the type but of the attribute, and the entry of the attribute says it. The value is compared character by character like an identifier, and it has to be an identifier the model defines: a reference to an identifier which is defined nowhere is a dangling reference, which makes the file invalid and leaves a simulation without a quantity it needs.
...
Every formula the report shows is of this kind: the kinetic law of a reaction, the formula of a rule, of an initial assignment, of a constraint and of a function definition, and the trigger, the delay and the priority of an event. A table and the inspector show the typeset formula, the infix formula is its tooltip and what a click copies, and a formula too long to typeset is shown as text until a click renders it. [Rendered math](concepts.md#rendered-math) describes this, and every element a formula names becomes a link of the kind [math](links.md#math).

## `ModelHistory` {#modelhistory}

Who created an element and when it was changed.

The standard annotation of an element may carry its history, written in RDF with the terms of Dublin Core: the creators with their name, their organisation and their electronic mail address, the date the element was created and one date for every time it was changed. The report reads that block out of the annotation and keeps it as one value of the element, instead of leaving it in the XML for a reader to decipher.
